...
int leaseWeightCapacity(vector<int> &a, int n, int days){
    int maxWeight = *max_element(a.begin(), a.end()); // Used STL function to find max element
    int sumWeight = accumulate(a.begin(), a.end(), 0); // Used STL function to find sum of elements

    for(int i = maxWeight; i <= sumWeight; i++){
        int reqDays = findDays(a, n, i);
        if(reqDays <= days){
            return i; // Return the minimum capacity that can ship within D days
        }
    }
